...
# ==========================================
# SET AUTOEXEC DELTA FROM GITHUB
# ==========================================
echo -e "${CYAN}[*] Mengunduh Kaitun.sh dari GitHub dan menyiapkan Autoexec Delta...${NC}"
DELTA_DIR="/sdcard/Delta/autoexec"
RUN "mkdir -p $DELTA_DIR"
RUN "rm -f $DELTA_DIR/*" # Bersihkan autoexec lama

# Unduh Kaitun.sh terbaru dari repo GitHub control-acl (Branch: MLBB)
curl -sL "https://raw.githubusercontent.com/AciLNiBoss/control-acl/refs/heads/MLBB/Kaitun.sh" > Kaitun.sh

# Verifikasi file berhasil diunduh dan tidak kosong
if [ -f "Kaitun.sh" ] && [ -s "Kaitun.sh" ]; then
    RUN "cp Kaitun.sh $DELTA_DIR/Kaitun.lua"
    echo -e "${GREEN}[+] Script Kaitun terbaru berhasil diunduh dan dipasang ke Autoexec Delta!${NC}"
else
    echo -e "${GREY}[!] Gagal mengunduh Kaitun.sh dari GitHub. Pastikan koneksi internet lancar atau link repo benar.${NC}"
fi
sleep 1
# ==========================================
...
# Logo dari GitHub repository AciLNiBoss
LOGO_URL="https://raw.githubusercontent.com/AciLNiBoss/control-acl/refs/heads/MLBB/68747470733a2f2f692e6962622e636f2f384b5a6b4d42622f313634393237363631373136322e6a7067.jpeg"
...
while true; do
    ...
    # --- FITUR 3: Cache Cleaner & Deep RAM Optimization ---
    if [ $((NOW - TIMER)) -ge 300 ]; then
        for PKG in $APPS; do RUN "rm -rf /data/data/$PKG/cache/*" >/dev/null 2>&1; done
        RUN "echo 3 > /proc/sys/vm/drop_caches" # Ini kode saktinya!
